module yousuinfts::freemintv2 {
    ...
    public entry fun create_freemint(freemint: &mut FreeMint, name: String, mint_per_address: u64, max_mint: u64, is_whitelist: bool, ctx: &mut TxContext) {
        ...
        let newfreemint = FreeMint {
            ...
            name: name,
            active:true,
            image_og: freemint.image_og,
            image_tier1: freemint.image_tier1,
            image_tier2: freemint.image_tier2,
            image_tier3: freemint.image_tier3,
            image_tier4: freemint.image_tier4,
            image_tier5: freemint.image_tier5,
            image_pfp1: freemint.image_pfp1,
            image_pfp2: freemint.image_pfp2,
            image_pfp3: freemint.image_pfp3,
            image_pfp4: freemint.image_pfp4,
    ...
    public entry fun create_freemint2(name: String, mint_per_address: u64, max_mint: u64, is_whitelist: bool, ctx: &mut TxContext) {
    ...
    public entry fun update_active(new: bool, freemint: &mut FreeMint, ctx: &mut TxContext){ 
        assert!(freemint.creator == tx_context::sender(ctx), ENotAuthorized);
        freemint.active = new;
    }
    ...
    public entry fun update_image(
        ...
        if(type == 0)
        {
             freemint.image_og = image_url
        };
        if(type == 1)
        {
             freemint.image_tier1 = image_url
        };
        if(type == 2)
        {
             freemint.image_tier2 = image_url
        };
        if(type == 3)
        {
             freemint.image_tier3 = image_url
        };
        if(type == 4)
        {
             freemint.image_tier4 = image_url
        };
        if(type == 5)
        {
             freemint.image_tier5 = image_url
        };
        if(type == 6)
        {
            if(index == 1 )
            freemint.image_pfp1 = image_url;
            if(index == 2 )
            freemint.image_pfp2 = image_url;
            if(index == 3 )
            freemint.image_pfp3 = image_url;
            if(index == 4 )
            freemint.image_pfp4 = image_url;
        };
    ...
    fun get_image_url(type:vector<u8>, freemint: &mut FreeMint):vector<u8> {
        ...
        if(type == b"5") {
            image_url = *bytes(&freemint.image_tier5);
        };

        if(type == b"4") {
            image_url = *bytes(&freemint.image_tier4);
        };

        if(type == b"3") {
            image_url = *bytes(&freemint.image_tier3);
        };

        if(type == b"2") {
            image_url = *bytes(&freemint.image_tier2);
        };

        if(type == b"1") {
            image_url = *bytes(&freemint.image_tier1);
        };

         if(type == b"og"){
            image_url = *bytes(&freemint.image_og);
        };

        if(type == b"pfp"){
   
            if(freemint.number %4 == 0)
            image_url = *bytes(&freemint.image_pfp4);
            if(freemint.number %4 == 1)
            image_url = *bytes(&freemint.image_pfp1);
            if(freemint.number %4 == 2)
            image_url = *bytes(&freemint.image_pfp2);
            if(freemint.number %4 == 3)
            image_url = *bytes(&freemint.image_pfp3);
        };
    ...
    public entry fun freemint(freemint: &mut FreeMint, nftinfo: &mut nft::Infomation  ,ctx: &mut TxContext) {
        ...
        assert!(freemint.active == true, ENotActive);
